...
int main(int argc, char* argv[]) {
    ...
    // Вывод результатов
    std::cout << "Оптимальные смешанные стратегии диспетчера: ";
    for (const auto& i : strategies_dispatcher)
        std::cout << std::to_string(i) << ' ';
    std::cout << '\n';

    std::cout << "Оптимальные смешанные стратегии ВЦ: ";
    for (const auto& i : strategies_vc)
        std::cout  << std::to_string(i) << ' ';
    std::cout << '\n';

    std::cout << "Цена игры V: " << V << endl;
    std::cout << "Количество итераций: " << iterations << endl;

    auto end = chrono::high_resolution_clock::now();
    chrono::duration<double> elapsed = end - start;

    // Запись времени выполнения в файл
    std::cout << "Execution time: " << elapsed.count() << " seconds" << endl;
