...
int main() {
  ...
  struct client_info clients[MAX_CLIENTS] = {
      0}; // Массив для хранения адресов клиентов
  char *strings[10];

  // Выделение памяти для каждой строки
  for (int i = 0; i < 10; i++) {
    strings[i] =
        malloc(32 * sizeof(char)); // Выделяем 20 символов для каждой строки
    if (strings[i] == NULL) {
  ...
  while (1) {
    ...
    if (fds.revents & POLLIN) {
      ...
      for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clients[i].active &&
            memcmp(&clients[i].addr, &clientAddr, sizeof(clientAddr)) == 0) {
          clientIndex = i;
          break;
        }
      }
      ...
      if (clientIndex == -1) {
        printf("SERVER: Список клиентов переполнен, игнорируем сообщение от "
               "%s:%d\n",
               inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port));
        sendto(sockMain, "Ошибка", 32, 0, (struct sockaddr *)&clientAddr,
               clientAddrLen);
        continue;
      }
      if (strncmp(buf, "/login ", 7) == 0) {
        ...
        if ((strlen(login) == 0) && (strlen(password) == 0)) {
          sendto(sockMain, "Ошибка в пароле.", 32, 0,
        ...
        if (check_credentials(login, password, "credentials.txt") == 1) {
          ...
          printf("SERVER: Клиент %s:%d успешно авторизован\n",
                 inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port));
          sendto(sockMain, "Вы зашли", 32, 0, (struct sockaddr *)&clientAddr,
                 clientAddrLen);
          continue;
          ...
          sendto(sockMain, "Неверный пароль.", 32, 0,
                 (struct sockaddr *)&clientAddr, clientAddrLen);
      ...
      if (!clients[clientIndex].active ||
          clients[clientIndex].login[0] == '\0' ||
          clients[clientIndex].password[0] == '\0' ||
          !clients[clientIndex].authenticated) {
        printf("SERVER: Клиент %s:%d не авторизован \n",
               inet_ntoa(clientAddr.sin_addr), ntohs(clientAddr.sin_port));
        sendto(sockMain, "Авторизуйтесь", 32, 0, (struct sockaddr *)&clientAddr,
               clientAddrLen);
      ...
      if (strcmp(buf, "/user") == 0) {
        for (int i = 0; i < MAX_CLIENTS; ++i) {
          ...
          else if ((sendto(sockMain, strings[i], 32, 0,
                           (struct sockaddr *)&clientAddr, clientAddrLen)) <
                   0) {
      ...
      // Отправка сообщения всем клиентам, кроме только что получившего
      // сообщение
      for (int i = 0; i < MAX_CLIENTS; ++i) {
        if (clients[i].active && i != clientIndex) {
          if ((sendto(sockMain, "/login", 32, 0,
                      (struct sockaddr *)&clients[i].addr, clientAddrLen)) <
              0) {
            perror("Ошибка при отправке сообщения клиенту");
            exit(1);
          }
          if ((sendto(sockMain, strings[clientIndex], 32, 0,
                      (struct sockaddr *)&clients[i].addr, clientAddrLen)) <
              0) {
            perror("Ошибка при отправке сообщения клиенту");
            exit(1);
          }
          if ((sendLength = sendto(sockMain, buf, msgLength, 0,
