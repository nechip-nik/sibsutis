...
void *send_msg(void *arg) {
    while (1) {
        ...
        if (strcmp(message, "/exit") == 0) {
            sendto(sock, "/exit", strlen(message), 0, (struct sockaddr *)&server, sizeof(server));
        exit(0); // Завершаем программу с кодом возврата 0
    }
        

        // Отправка сообщения на сервер
        if (sendto(sock, message, strlen(message), 0, (struct sockaddr *)&server, sizeof(server)) == -1) {
            perror("sendto()");
            break;
        }
...
int main(void) {
    printf("Войдите в аккаунт или зарегистрируйтесь используя \"/login\" <login> <password>\n \"/help\" - список команд.\n");
    // Создание сокета
